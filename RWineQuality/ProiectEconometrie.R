...
# setup
rm(list = ls()) 
directory <- "C:/Users/gigel/OneDrive/Documente/Proiect/"
...
# Explorarea datelor - covarianta si media
cor(wine)
...
(se <- vcov(model_wine) %>% # matricea de varianta-covarianta
    ...
    sqrt %>% # radical
    .["alcohol"]) # obtinerea erirolor stadarde (se) ale regresorului "exper"
...
# price = -2.177e+01 + 2.068e-03 * lotsize + 1.228e-01 * sqrft + 1.385e+01 * bdrms
...
(se2 <- vcov(model_multiple) %>%
    diag %>% 
    sqrt %>% 
    .["sulphates"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
(se3 <- vcov(model_multiple) %>%
    diag %>% 
    sqrt %>% 
    .["volatile_acidity"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
# model restrictionat: wage = alpha0 + alpha1*educ + alpha4*female + e
...
wine_dummy <- wine_dummy %>% mutate(lquality = log(quality),
...
wine_dummy %<>% mutate(whiteXlalcohol = white*lalcohol)
model_dummy <- lm(lquality ~ lalcohol + lvolatile_acidity + lsulphates + white + whiteXlalcohol, wine_dummy)
...
(se1 <- vcov(model_dummy) %>%
    diag %>% 
    sqrt %>% 
    .["lalcohol"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
(se2 <- vcov(model_dummy) %>%
    diag %>% 
    sqrt %>% 
    .["lsulphates"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
(se3 <- vcov(model_dummy) %>%
    diag %>% 
    sqrt %>% 
    .["lvolatile_acidity"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
(se4 <- vcov(model_dummy) %>%
    diag %>% 
    sqrt %>% 
    .["white"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
(se5 <- vcov(model_dummy) %>%
    diag %>% 
    sqrt %>% 
    .["whiteXlalcohol"]) # obtinerea erirolor stadarde (se) ale regresorului "alcohol"
...
model <- lm(lquality ~ lalcohol + lvolatile_acidity + lsulphates, wine_dummy)
summary(model)
...
y_pred_outsample
exp(y_pred_outsample) # operatia opusa log pentru a vedea calitatea prognozata si nu procente 
...
wage1 <- read.csv(paste0(directory, "wine_quality1.csv"))
# Modelul de regresie pentru salariu

model1 <- lm(quality ~ alcohol + sulphates + volatile_acidity + white, wage1)
# Modelul de regresie pentru salariu
summary(model1)

wage1 %<>% mutate(yhat = fitted(model1), # reziduuri
...
# p-value<0.1 => modelul nu este bine specificat
